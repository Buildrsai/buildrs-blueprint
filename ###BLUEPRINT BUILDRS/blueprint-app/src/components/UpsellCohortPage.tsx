...
const AGENTS = [
  ...
  },
  {
    ...
    color: "#8b5cf6",
    unlimited: false,
    desc: "Génère les prompts Claude Code parfaits — phase par phase. Debug en temps réel.",
  },
  {
    Robot: RobotLauncher,
    name: "Launcher",
    color: "#14b8a6",
    unlimited: false,
    desc: "Déploie ton app, rédige ta landing, crée tes emails et lance ta première campagne.",
  },
]
...
export function UpsellCohortPage({ onDecline }: Props) {
  ...
  const displayPrice = expired ? 497 : 197

  const handlePay = async () => {
    setLoading(true)
    setError(null)
    try {
      // Récupère le session_id du premier achat depuis l'URL pour pré-remplir Stripe
      const hashParams = new URLSearchParams(window.location.hash.split('?')[1] ?? '')
      const priorSessionId = hashParams.get('session_id') ?? undefined

      const res  = await fetch(`${SUPABASE_FUNCTIONS_URL}/create-agents-checkout`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ prior_session_id: priorSessionId }),
      })
  ...
  const title = isClaude
    ...
    : "Tu as le plan. Voici l'équipe qui exécute."
          ...
          <>
            ...
            <div className="mb-10 text-center">
              ...
              <p className="mx-auto max-w-[520px] text-[15px] leading-[1.75] text-muted-foreground">
                7 agents IA spécialisés qui produisent tes livrables à chaque étape. Ton cahier des charges, ton design, ton architecture, ton code, ton lancement — en quelques minutes au lieu de quelques jours.
              </p>
            </div>
            ...
            <div className="rounded-2xl border border-border bg-card overflow-hidden mb-5">
              ...
              <div className="px-7 pb-7">
                <div className="mb-1 flex items-baseline gap-3">
                  {!expired && (
                    <span className="text-[16px] font-medium text-muted-foreground/40 line-through">497€</span>
                  )}
                ...
                <p className="mb-6 text-[12px] text-muted-foreground/60">
                  {expired
                    ? "Offre de lancement terminée — tarif standard."
                    : "Soit 39€ par agent. Un freelance te coûterait 500€ par livrable."}
                </p>
