...
const sizeMap: Record<FolderSize, {
  container: string;
  tabLeft: string;
  tabRight: string;
  tabBridge: string;
  ...
  sm: {
    container: "w-24 h-24 rounded-[24px]",
  ...
  md: {
    container: "w-32 h-32 rounded-[32px]",
    tabLeft: "w-12 h-4 rounded-tl-lg",
    tabRight: "w-3 h-4 rounded-tr-[32px]",
    tabBridge: "w-3 h-3",
    flapBody: "h-12",
...
const colorMap: Record<FolderColor, {
  folder: string;
  flap: string;
  paperBack: string;
  paperFront: string;
  ...
  black: {
    folder: "from-neutral-700 to-neutral-900",
...
export function Folder({ color = "blue", size = "md", label, className }: FolderProps) {
    ...
    >
      ...
      <div className="absolute right-0 bottom-0 left-0 z-20">
        <div className="flex items-end">
          ...
          <div className={cn(s.tabBridge, c.flap)} />
        </div>
