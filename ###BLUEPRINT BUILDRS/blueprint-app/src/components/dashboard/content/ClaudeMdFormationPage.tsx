  ...
  return (
    <div className="flex gap-4 p-4 rounded-xl" style={{ background: 'rgba(255,255,255,0.02)', border: '0.5px solid rgba(255,255,255,0.07)' }}>
      ...
      <div className="flex-1 pb-1">
        <p className="text-[13px] font-bold mb-0.5" style={{ color: '#f0f0f5', letterSpacing: '-0.01em' }}>{title}</p>
        <code className="text-[11px]" style={{ fontFamily: 'Geist Mono, monospace', color: '#8b5cf6' }}>{path}</code>
        <p className="text-[12px] mt-1.5 leading-relaxed" style={{ color: '#5b6078' }}>{desc}</p>
...
function CompareRow({ bad, good }: { bad: string; good: string }) {
  return (
    <div className="grid grid-cols-2 gap-2 py-2" style={{ borderBottom: '0.5px solid rgba(255,255,255,0.05)' }}>
      <div className="flex items-start gap-1.5">
        <X size={10} strokeWidth={2} style={{ color: '#ef4444', flexShrink: 0, marginTop: 2 }} />
        <p className="text-[11px]" style={{ color: '#5b6078' }}>{bad}</p>
      </div>
      <div className="flex items-start gap-1.5">
        <Check size={10} strokeWidth={2} style={{ color: '#22c55e', flexShrink: 0, marginTop: 2 }} />
        <p className="text-[11px]" style={{ color: '#9399b2' }}>{good}</p>
      </div>
    </div>
  )
}
  ...
  return (
    <div className="flex-1 overflow-y-auto">
      <div className="max-w-3xl mx-auto px-6 py-8">
        ...
        <Body>
          Sans CLAUDE.md, voici ce qui se passe à <strong style={{ color: '#f0f0f5' }}>chaque nouvelle session</strong> Claude Code :
        </Body>
        ...
        <div className="rounded-xl p-4 my-4 space-y-2" style={{ background: 'rgba(239,68,68,0.04)', border: '0.5px solid rgba(239,68,68,0.15)' }}>
          {[
            ...
            <div key={t} className="flex items-center gap-2">
              ...
              <p className="text-[12px]" style={{ color: '#5b6078' }}>{t}</p>
            </div>
          ))}
        </div>

        <Body>
          Avec CLAUDE.md, tout ça est chargé <strong style={{ color: '#f0f0f5' }}>automatiquement</strong>. Claude Code lit ce fichier au démarrage de chaque session. Il connaît déjà ton projet, ta stack, tes conventions, tes commandes. Il respecte tes règles dès la première ligne de code générée.
        </Body>

        <SubTitle title="C'est quoi concrètement ?" />
        <Body>
          Un fichier texte en Markdown, nommé <code style={{ fontFamily: 'Geist Mono, monospace', fontSize: '11px', padding: '1px 4px', background: 'rgba(255,255,255,0.08)', borderRadius: 4 }}>CLAUDE.md</code>, placé à la racine de ton projet. Claude Code le lit automatiquement à chaque lancement. C'est ta <strong style={{ color: '#f0f0f5' }}>mémoire persistante</strong> — le briefing permanent que tu donnes à ton assistant.
        </Body>

        <Callout type="success">
          Chez Buildrs, le CLAUDE.md est la <strong style={{ color: '#f0f0f5' }}>première chose qu'on crée</strong> dans chaque projet. Avant d'écrire une ligne de code. Avant de demander quoi que ce soit à Claude Code. Un bon CLAUDE.md = du code production-grade dès le premier prompt.
        </Callout>
        ...
        <Body>
          Claude Code ne se contente pas d'un seul fichier. Il utilise un système à plusieurs niveaux. Les instructions les plus <strong style={{ color: '#f0f0f5' }}>spécifiques</strong> l'emportent toujours sur les plus génériques.
        </Body>
...
- Pas de console.log en production`}
        ...
        <Body>
          Tu peux référencer d'autres fichiers directement depuis ton CLAUDE.md avec la syntaxe <code style={{ fontFamily: 'Geist Mono, monospace', fontSize: '11px', padding: '1px 4px', background: 'rgba(255,255,255,0.08)', borderRadius: 4 }}>@chemin/vers/fichier</code>. C'est ce qui évite de dupliquer la documentation.
        </Body>
...
Voir @package.json pour les scripts disponibles.`}
        ...
        <div className="grid grid-cols-2 gap-3 my-4">
          {[
            ...
            <div key={item.title} className="p-3.5 rounded-xl" style={{ background: 'rgba(255,255,255,0.02)', border: '0.5px solid rgba(255,255,255,0.07)' }}>
              <p className="text-[12px] font-bold mb-1" style={{ color: item.color }}>{item.title}</p>
              <p className="text-[11px] leading-relaxed" style={{ color: '#5b6078' }}>{item.desc}</p>
            </div>
          ))}
        </div>

        <Callout>
          Les imports supportent jusqu'à <strong style={{ color: '#f0f0f5' }}>5 niveaux de profondeur</strong> récursifs. Les imports dans les blocs de code (backticks) sont ignorés — pas de risques d'effets de bord.
        </Callout>
        ...
        <Body>
          En plus du CLAUDE.md que tu rédiges, Claude Code possède une <strong style={{ color: '#f0f0f5' }}>auto-mémoire</strong>. Au fil de tes sessions, il note automatiquement ce qu'il apprend sur ton projet.
        </Body>
...
└── ...                 # Autres fichiers thématiques`}
        ...
        <div className="space-y-2 my-4">
          {[
            ...
            <div key={item.label} className="flex items-center gap-3 py-2" style={{ borderBottom: '0.5px solid rgba(255,255,255,0.05)' }}>
...
"Note que les tests API nécessitent un Redis local"`}
        ...
        <Callout>
          Pour consulter ou modifier la mémoire automatique, tape <code style={{ fontFamily: 'Geist Mono, monospace', fontSize: '11px', padding: '1px 4px', background: 'rgba(255,255,255,0.08)', borderRadius: 4 }}>/memory</code> dans Claude Code.
        </Callout>
...
- Pas de console.log en production`}
        ...
        <div className="rounded-xl overflow-hidden my-4" style={{ border: '0.5px solid rgba(255,255,255,0.08)' }}>
          <div className="grid grid-cols-2 px-4 py-2" style={{ borderBottom: '0.5px solid rgba(255,255,255,0.08)', background: 'rgba(255,255,255,0.03)' }}>
        ...
        <Body>
          Un point important que beaucoup ignorent : le CLAUDE.md <strong style={{ color: '#f0f0f5' }}>survit au /compact</strong>. Après une compaction, Claude Code re-lit ton CLAUDE.md depuis le disque et le réinjecte dans la session.
        </Body>

        <Callout type="warning">
          Si une instruction disparaît après un <code style={{ fontFamily: 'Geist Mono, monospace', fontSize: '11px', padding: '1px 4px', background: 'rgba(255,255,255,0.08)', borderRadius: 4 }}>/compact</code>, c'est qu'elle était dans la conversation, pas dans le CLAUDE.md. Morale : tout ce qui est important doit être dans le fichier, pas juste dit à l'oral.
        </Callout>
        ...
        <div className="mt-10 rounded-2xl p-6 text-center" style={{ background: 'rgba(139,92,246,0.05)', border: '0.5px solid rgba(139,92,246,0.2)' }}>
          <div className="flex items-center justify-center gap-2 mb-2">
            ...
            <p className="text-[14px] font-extrabold" style={{ color: '#f0f0f5', letterSpacing: '-0.02em' }}>Crée ton CLAUDE.md maintenant</p>
          </div>
